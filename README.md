...
```bash
$ npm install react-native-slient-pay --save
//or
$ yarn add react-native-slient-pay
```
...
$ cd ios && pod install
//or
$ react-native link react-native-slient-pay
```
...
2. Go to `node_modules` ➜ `react-native-slient-pay` and add `RNPutiPay.xcodeproj`
...
2. Append the following lines to `android/settings.gradle`:
   ...
   ```
      include ':react-native-slient-pay'
      project(':react-native-slient-pay').projectDir = new File(rootProject.projectDir,     '../node_modules/react-native-slient-pay/android')
   ```
3. Insert the following lines inside the dependencies block in `android/app/build.gradle`:

   ```
      compile project(':react-native-slient-pay')
   ```
...
```javascript
      import XPay from 'react-native-puti-pay'
        ...
        XPay.setWxId(id, 'https://your.universalLink')
        //设置    支付宝URL Schemes
